...
LORA_PATHS = {
    'bias': "./lora_bias.safetensors",     # LoRAbias 
    'fair': "./lora_fair.safetensors",     # LoRAfair
    'self_po': "./lora_self_po.safetensors" # LoRASelf-PO
}
...
def set_adapter_weights(pipe, alpha=0.5, beta=0.5, use_lora=True):
    """
    ϵfinal = ϵbias + α(ϵfair - ϵbias) + β(ϵself_po - ϵbias)
    
    - lora_bias: 1.0 (reference point)
    - lora_fair: alpha 
    - lora_self_po: beta
    """
    if use_lora:
        ...
        weights = [1.0, alpha, beta]
        
        try:
            pipe.set_adapters(adapters, adapter_weights=weights)
            print(f"Adapter weights set - α: {alpha}, β: {beta}")
        except:
...
def generate_skin_disease_prompt(disease_name, target_attributes=None):
    """    
    Args:
        disease_name: (e.g. 'psoriasis', 'acne', 'rosacea')
        target_attributes: {'race', 'gender', 'age'} 
    """
...
def generate_anonymized_image(image_path, mask_path, disease_name, 
    ...
    Args:
        ...
        mask_path,
        disease_name, 
        target_attributes: demographic attributes
        alpha: fairness control (0~1)
        beta: consistency control (0~1)
    """
    try:
...
if __name__ == "__main__":
    ...
    generated_image = generate_anonymized_image(
        ...
        alpha=0.5,  # attribute strength
        beta=0.5,   # consistency strength
        pipe=sd_pipe
    )
