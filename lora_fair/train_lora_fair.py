...
def parse_args():
    ...
    parser.add_argument("--fairface_val_metadata", type=str, default="")
    ...
    parser.add_argument("--fairface_train_images", type=str, required=True)
    parser.add_argument("--fairface_val_images", type=str, default="")
...
class CombinedFairnessDataset(Dataset):
    """Dataset for LoRAfair training"""
    def __init__(self, clinical_path, fairface_path, 
                 clinical_img_dir, fairface_img_dir,
                 resolution=512):
        self.data = []
        ...
        if clinical_path and os.path.exists(clinical_path):
            with open(clinical_path, 'r') as f:
                ...
                for item in clinical_data:
                    image_path = os.path.join(clinical_img_dir, item['image_path'])
                ...
                logger.info(f"Loaded {len(self.data)} clinical samples")
        ...
        if fairface_path and os.path.exists(fairface_path):
            with open(fairface_path, 'r') as f:
                ...
                for item in fairface_data:
                    image_path = os.path.join(fairface_img_dir, item['image_path'])
                    if not os.path.exists(image_path):
                        logger.warning(f"Image not found: {image_path}")
                        continue
                    ...
                    self.data.append({
                        'image_path': image_path,
                        'prompt': f"A {item['age']} years old {item['race']} {item['gender']} face",
                        'metadata': {
                            'gender': item['gender'],
                            'age': item['age'],
                            'race': item['race'],
    ...
    while global_step < args.max_train_steps:
        ...
        for batch in train_dataloader:
            ...
            with accelerator.accumulate(unet):
                ...
                latents = latents.repeat_interleave(args.images_per_prompt, dim=0)
                
                # Expand prompts similarly
                expanded_prompts = []
                ...
                if noise_scheduler.config.prediction_type == "epsilon":
                    ...
                    # x0 = (xt - sqrt(1-alpha_t) * epsilon) / sqrt(alpha_t)
                ...
                with torch.no_grad():
                    ...
                    generated_images = vae.decode(pred_original_sample).sample
                    generated_images = generated_images.clamp(-1, 1)
                
                demographic_logits = classifiers(generated_images)
